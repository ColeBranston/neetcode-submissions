...
class LRUCache:
    ...
    '''
        ...
        Insert uses the right most pointer and connects the prevous last node to the incoming node, and the prev
        pointer node to the prevous last node, then the incoming node to the lru right pointer and the right
        ...
        and cache tracking would break. From there the node is re-cached by settting the key to the object: Node(key, val),
