...
class Solution:
    def reverseKGroup(self, head: Optional[ListNode], k: int) -> Optional[ListNode]:
        ...
        while True:
            ...
            # Further explanation is that on first time, start is ListNode(0 (dummy val), head (start of LL)), therefore making it so that reversal doesn't show in the dummy reference until its changed with start.next = kth node, so while the reversal does occur, the start.next still points to ListNode(1) and therefore would delete the reversed nodes if wasn't rewired with start.next = kth (now first node in the reversed LL), since the last node will always point to the stop node, all you have to do is set start's next pointer to the new first node in the group and the other end will already have been figured out.
            start = temp
